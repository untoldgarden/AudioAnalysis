...
namespace UntoldGarden.AudioAnalysis
{
    ...
    public class AudioDataToVFX : MonoBehaviour
    {
        ...
        [SerializeField] List<VisualEffect> _visualEffects;
        ...
        void Update()
        {
            if (analyzer != null)
            {
                ...
                {
                    if (bindSpectrumTexture)
                        vfx.SetTexture(spectrumTextureName, _spectrumTexture);
                    if (bindWaveformTexture)
                        vfx.SetTexture(waveformTextureName, _waveformTexture);
                    if (bindRMS)
                        vfx.SetFloat(rmsName, analyzer.RmsValue);
                    if (bindDbValue)
                        vfx.SetFloat(dbName, analyzer.DbValue);
                    if (bindPitch)
                        vfx.SetFloat(pitchName, analyzer.PitchValue);
                }
            }
